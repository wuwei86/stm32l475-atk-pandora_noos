...
lv_obj_t *main_win_contanier = NULL;
...
{
    main_win_contanier = lv_obj_create(lv_scr_act());  // 基于屏幕创建一个和屏幕大小一样的容器win1_contanier
    if (main_win_contanier != NULL)
    {
        //lv_obj_set_style_bg_color(main_win_contanier, lv_palette_main(LV_PALETTE_BLUE), 0);
        lv_obj_set_size(main_win_contanier, 240, 240);
        ...
        img1 = lv_img_create(main_win_contanier);
